...
3. Set up ESLint and TypeScript to work together
   ...
   And then configure ESLint to use that parser and parse only TypeScript files in the project. In `/package.json`,
   adapt
   ...
   And then adapt the ESLint configuration in `/package.json` (again, be careful on the typescript-eslint plugin
   position
...
5. Modify the `tsconfig.json` file to avoid compiling the `.steps.ts` and `.test.ts` files while building the
   distribution:
...
In order to generate a clean build, I want to clean up any previously compiled files from the distribution folder. Since
Window
doesn't offer a clean "rm -rf" command, I have opted for a simple trick: delete the whole dist folder and recreate it,
empty.
